...
Var
  ...
  J - counter 2
  K - counter 3
  Checker - variable, which is used to find mistakes and to
   delete words with repeating letters }
...
Begin
  ...
  repeat
    ...
    //Check condition of only english letters,
    //allowing input spaces and сommas
    if length(Str) <> 0 then
    begin
      for I := 1 to length(Str) - 1 do
      begin
        if ((ord(Str[I]) < 96) or (ord(Str[I]) > 125)) and
   ...
   (pos(',',Str) = 0)) or (Checker = false) or
   (Str[length(Str)] <> '.') then
      writeln('Ошибка ввода');

  //Check if input is sufficient
  until (Checker = true) and (Str[length(Str)] = '.') and
        (Str <> '') and ((pos(' ',Str) <> 0) or
        (pos(',',Str) <> 0));
  ...
  begin
    ...
    begin
      ...
      //If found word equals to the last word or there
      //are repeated lettters
